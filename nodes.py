...
class CombineTutorial(Node):
    def prep(self, shared):
        ...
        # --- Generate Combined Content if requested ---
        combine = shared.get("combine", False)
        combined_content = None
        if combine:
            combined_content = f"# Tutorial: {project_name}\n\n"
            combined_content += f"{relationships_data['summary']}\n\n"
            combined_content += f"**Source Repository:** [{repo_url}]({repo_url})\n\n"

            # Add Mermaid diagram for relationships
            combined_content += "```mermaid\n"
            combined_content += mermaid_diagram + "\n"
            combined_content += "```\n\n"

            combined_content += f"## Table of Contents\n\n"
            for i, abstraction_index in enumerate(chapter_order):
                if 0 <= abstraction_index < len(abstractions) and i < len(chapters_content):
                    abstraction_name = abstractions[abstraction_index]["name"]
                    # Generate safe slug for internal links (GitHub / VS Code format)
                    raw_slug = f"chapter-{i+1}-{abstraction_name}"
                    safe_link = "#" + "".join(
                        c.lower() if c.isalnum() or c == " " or c == "-" else "" for c in raw_slug
                    ).replace(" ", "-")
                    while "--" in safe_link:
                        safe_link = safe_link.replace("--", "-")
                    safe_link = safe_link.rstrip("-")
                    combined_content += f"{i+1}. [{abstraction_name}]({safe_link})\n"

            combined_content += "\n---\n\n"

            # Append each chapter content
            for i, abstraction_index in enumerate(chapter_order):
                if 0 <= abstraction_index < len(abstractions) and i < len(chapters_content):
                    abstraction_name = abstractions[abstraction_index]["name"]
                    chapter_content = chapters_content[i]
                    # Format title
                    combined_content += f"# Chapter {i+1}: {abstraction_name}\n\n"
                    combined_content += chapter_content
                    if not combined_content.endswith("\n\n"):
                        combined_content += "\n\n"
                    combined_content += "---\n\n"

            combined_content += f"Generated by [AI Codebase Knowledge Builder](https://github.com/The-Pocket/Tutorial-Codebase-Knowledge)"
    ...
    def exec(self, prep_res):
        ...
        combine = prep_res.get("combine", False)
        combined_content = prep_res.get("combined_content")
        ...
        if combine and combined_content:
            combined_filepath = os.path.join(output_path, "combined_tutorial.md")
